package org.freedomtool.view.util.segmentetProgressBar
...
class SegmentedProgressBar : View, Runnable, ViewPager.OnPageChangeListener, View.OnTouchListener {
    ...
    var segmentSelectedBackgroundColor: Int = context.getThemeColor(android.R.attr.colorAccent)
    ...
    fun updateSegmentedSelectedColor(color: Int) {
        segmentSelectedBackgroundColor = resources.getColor(
            color
        )
    }
    ...
    constructor(context: Context, attrs: AttributeSet) : super(context, attrs) {
        ...
        margin = typedArray.getDimensionPixelSize(
            R.styleable.SegmentedProgressBar_segmentMargins, margin
        )
        radius = typedArray.getDimensionPixelSize(
            R.styleable.SegmentedProgressBar_segmentCornerRadius, radius
        )
        segmentStrokeWidth = typedArray.getDimensionPixelSize(
            R.styleable.SegmentedProgressBar_segmentStrokeWidth, segmentStrokeWidth
        )

        segmentBackgroundColor = typedArray.getColor(
            R.styleable.SegmentedProgressBar_segmentBackgroundColor, segmentBackgroundColor
        )
        segmentSelectedBackgroundColor = typedArray.getColor(
            R.styleable.SegmentedProgressBar_segmentSelectedBackgroundColor,
            segmentSelectedBackgroundColor
        )

        segmentStrokeColor = typedArray.getColor(
            R.styleable.SegmentedProgressBar_segmentStrokeColor, segmentStrokeColor
        )
        segmentSelectedStrokeColor = typedArray.getColor(
            R.styleable.SegmentedProgressBar_segmentSelectedStrokeColor, segmentSelectedStrokeColor
        )

        timePerSegmentMs = typedArray.getInt(
            R.styleable.SegmentedProgressBar_timePerSegment, timePerSegmentMs.toInt()
        ).toLong()
    ...
    fun setProgress(progress: Int) {
        selectedSegment!!.animationProgress = progress
    }

    fun start() {
        pause()
        if (selectedSegment == null) next()
     ...
     */
    fun setPosition(position: Int, userAction: Boolean = true) {
        loadSegment(offset = position - this.selectedSegmentIndex, userAction = userAction)
    }
